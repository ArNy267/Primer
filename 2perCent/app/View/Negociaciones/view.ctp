...
<h2><?php echo __('Negociacion'); ?></h2>
	...
	<br />
	<dl>	
		<?php echo $this->Html->link(__('Ver solicitud'), array(
				'controller' => 'Solicitudes', 'action' => 'view', $negociacion['Solicitud']['id'] ))."<br />"; ?>

		<?php /*ebug($Comunicacion); */foreach ($Comunicacion as $item ) { ?>
			<?php $item['Usuario']['id'] ?>
			
			<dt> <?php echo __('De'); ?></dt>
		 	<dd> <?php echo h($item['Usuario']['nombre']); ?> 	&nbsp; 	</dd>

			<dt> <?php echo __('Mensaje'); ?></dt>
			<dd> <?php echo h($item['Negociacion']['mensaje']); ?> 	&nbsp; 	</dd>
			<dt> <?php echo __('Fecha'); ?></dt>
			<dd> <?php echo h($item['Negociacion']['created']); ?> 	&nbsp; 	</dd>

		<?php	
		}  ?>

		<?php if ($Comunicacion == null ) {

				if($negociacion['Negociacion']['usuario_id'] != $usuario_logged_id) {  

				 echo $this->Html->link(__('Responder'), array(
						'controller' => 'Negociaciones', 'action' => 
						'Responder', h($negociacion['Negociacion']['usuario_id']),h($negociacion['Negociacion']['solicitud_id']) ))."<br />";
				 echo $this->Html->link(__('Aceptar'), array(
						'ontroller' => 'Negociaciones', 'action' => 'add' )) ;  
					} 

				}
				 else 
					 { 
					 	if(h($negociacion['Negociacion']['usuario_id2'] == $usuario_logged_id )){
					 		echo $this->Html->link(__('Responder Mensaje'), array(
							'controller' => 'Negociaciones', 'action' => 'Responder', h($negociacion['Negociacion']['usuario_id']),h($negociacion['Negociacion']['solicitud_id'])  ));
					 		}
					 		else {
					 			echo $this->Html->link(__('Responder Mensaje'), array(
								'controller' => 'Negociaciones', 'action' => 'Responder', h($negociacion['Negociacion']['usuario_id2']),h($negociacion['Negociacion']['solicitud_id'])  ));
